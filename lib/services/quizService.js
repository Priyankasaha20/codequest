...
class QuizService {
  ...
  async startQuiz(subject, count = 10) {
    try {
      ...
      console.log("Starting quiz with:", requestBody);
      console.log("API URL:", `${API_BASE_URL}/api/quiz/start`);
  ...
  async getQuestion(sessionId) {
    try {
      ...
      console.log("API URL:", `${API_BASE_URL}/api/quiz/${sessionId}/question`);
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        console.error("Get question error:", errorData);
      ...
      const data = await response.json();
      console.log("Question loaded successfully:", data);
      return data;
    } catch (error) {
  ...
  async submitAnswer(sessionId, answerIndex) {
    try {
      ...
      console.log("API URL:", `${API_BASE_URL}/api/quiz/${sessionId}/answer`);
